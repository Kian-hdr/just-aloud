# Set up Just Aloud with your AI agent
...
```text
Install and set up Just Aloud on my Mac using the official project:
https://github.com/Kian-hdr/just-aloud
...
managed by Homebrew. Current requirements: macOS 13 or later; Apple Silicon or Intel. Local Kokoro additionally requires Apple Silicon.
...
Read https://github.com/Kian-hdr/just-aloud/releases/latest and the current README.
...
3. Install one copy.
For a verified downloaded app, copy Just Aloud.app into /Applications, preserving its
...
If using Homebrew, inspect the official tap https://github.com/Kian-hdr/homebrew-tap
and its actual cask URL, checksum, requirements, dependencies, and install hooks
before executing them. The tap command is brew tap Kian-hdr/tap; the fully qualified
install target is Kian-hdr/tap/just-aloud. Use brew install --cask for a new installation
...
bundle identifier (space.exlumina.justaloud), then inspect its visible interface when UI tools
are available. A running process alone is not a successful UI or feature test.
Ask whether I want ElevenLabs cloud speech or local Kokoro before configuring an
engine. For ElevenLabs, guide me to Settings > API Key in the app and let me enter
my own key directly there, where the app stores it in macOS Keychain. Never ask me
to paste a secret into chat or put it in a command, log, or config file. Explain
that cloud speech sends selected text to ElevenLabs and uses my provider credits.
Help me select a voice or enter my own voice ID. Do not create an account or buy
credits. For Kokoro on Apple Silicon, explain the additional model and Python
downloads, then use the app's documented local-engine setup after I choose it.
Do not install the source installer over the downloaded app.

Open Welcome & Setup and guide me through granting Accessibility in System
Settings for the Option+Shift+/ selected-text shortcut. Leave permission approval
to me. Do not migrate Speak11 settings unless I request it. With my agreement,
use a short harmless sentence to test the shortcut, audible speech, pause/resume,
and stop. Obtain my approval before a cloud test consumes provider credits.
